...
# run
run = wandb.init(project="non_deterministic_test", job_type="data_tests")


@pytest.fixture(scope="session")
def data():

    local_path = run.use_artifact("data_split/data_train.csv:latest").file()
    sample1 = pd.read_csv(local_path)

    local_path = run.use_artifact("data_split/data_test.csv:latest").file()
